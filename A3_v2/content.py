...
def logistic_cost_function(w, x_train, y_train):
    ...
    :return: function returns tuple (val, grad), where val is a value of logistic function and grad is its gradient
        (calculated for parameters w)
    """
...
def gradient_descent(obj_fun, w0, epochs, eta):
    ...
    where w is vector of optimal model parameters and func_valus is vector of objective function values [epochs x 1],
        calculated for each epoch
    """
    w = w0
    v, grad = obj_fun(w0)
    func_values = np.ndarray(shape=(epochs, 1))
...
def stochastic_gradient_descent(obj_fun, x_train, y_train, w0, epochs, eta, mini_batch):
    """
    :param obj_fun: objective function that is minimized. To call the function use expression "val,grad = obj_
    fun(w,x,y)", where x,y indicates mini-batches.
    ...
    where w is vector of optimal value of model parameters and func_valus is vector of objective function values
    [epochs x 1], calculated for each epoch.
    ...
    for i, j in enumerate(lambdas):
        ...
        for k, l in enumerate(thetas):
            ...
            if f > f_max:
                ...
                f_max = f
    f_max = F
    return theta, reg_lambda, v, f_max
